...
await useAsyncData("blog-detail-related", () => blogStore.getPublicBlogs({ page: 1 }));
...
const readTime = computed(() => {
  ...
  const words = publicBlog.value.content.replace(/<[^>]+>/g, "").split(/\s+/).filter(Boolean).length;
...
<template>
  <div>
    <!-- Loading -->
    <div v-if="publicLoading" class="max-w-3xl mx-auto px-4 py-16 animate-pulse space-y-6">
      ...
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 shrink-0" />
      ...
      <div class="space-y-3 pt-4">
        <div v-for="i in 8" :key="i" class="h-4 bg-gray-100 dark:bg-gray-800 rounded" :class="i % 4 === 0 ? 'w-3/4' : 'w-full'" />
      </div>
    </div>

    <!-- Not found -->
    <div v-else-if="notFound" class="max-w-2xl mx-auto px-4 py-32 text-center">
      <UIcon name="i-lucide-file-x" class="w-12 h-12 text-gray-200 dark:text-gray-700 mx-auto mb-4" />
      <p class="text-gray-600 dark:text-gray-400 font-medium mb-1">Article not found</p>
      <p class="text-gray-400 text-sm mb-8">This article doesn't exist or may have been removed.</p>
      <NuxtLink
        to="/blogs"
    ...
    <div v-else-if="publicBlog">
      ...
      <div class="border-b border-gray-100 dark:border-gray-800">
        <div class="max-w-3xl mx-auto px-4 pt-12 pb-10">
          ...
          >
            <UIcon name="i-lucide-arrow-left" class="w-4 h-4 group-hover:-translate-x-0.5 transition-transform" />
            All articles
          </NuxtLink>

          <!-- Title -->
          <h1 class="text-3xl sm:text-4xl lg:text-5xl font-bold leading-tight text-gray-900 dark:text-white mb-5">
            {{ publicBlog.title }}
          </h1>

          <!-- Excerpt -->
          <p v-if="publicBlog.excerpt" class="text-xl text-gray-500 dark:text-gray-400 leading-relaxed mb-8">
            {{ publicBlog.excerpt }}
          </p>

          <!-- Byline -->
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-800 overflow-hidden shrink-0 flex items-center justify-center">
              <img
            ...
            <div>
              <p class="text-sm font-semibold text-gray-700 dark:text-gray-200">{{ publicBlog.author?.name }}</p>
      ...
      <article class="max-w-2xl mx-auto px-4 py-12 sm:py-16">
        <div
          class="prose prose-gray dark:prose-invert max-w-none
                 ...
                 prose-blockquote:not-italic prose-blockquote:pl-5 prose-blockquote:pr-4
      ...
      <!-- More articles -->
      <div v-if="relatedBlogs.length > 0" class="border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-900">
        <UContainer class="max-w-5xl py-16">
          <div class="flex items-center justify-between mb-10">
            <h2 class="text-xl font-bold text-gray-900 dark:text-white">More Articles</h2>
            <NuxtLink
              to="/blogs"
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            >
              <div class="overflow-hidden rounded-xl w-full aspect-video bg-gray-100 dark:bg-gray-800 mb-4">
                <img
                  v-if="blog.thumbnail"
                  :src="blog.thumbnail"
                  :alt="blog.title"
              ...
              <h3 class="text-base font-bold text-gray-900 dark:text-white group-hover:text-gray-600 dark:group-hover:text-gray-300 transition-colors leading-snug">
                {{ blog.title }}
              </h3>
              <p v-if="blog.excerpt" class="text-gray-500 dark:text-gray-400 text-sm line-clamp-2 mt-1.5 leading-relaxed">
                {{ blog.excerpt }}
              </p>
            </NuxtLink>
          </div>
